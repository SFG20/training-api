...
registerAPI(
    /* GraphQL */ `
        type Query {
            ...
            getTodos(searchTerm: String, skip: Int, take: Int, status: TodoStatus, orderStmt: [String]): [Todo]
        }
...
function getTodos(_, { searchTerm = "", status, skip = 0, take = "10", orderStmt = ["title_nc_"] }) {
    try {
        const where = {
            $and: [!!searchTerm && { _nc_title: { $like: `%${searchTerm}%` } }, !!status && { status: status }],
        }
